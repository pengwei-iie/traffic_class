...
class RecognitionModel(nn.Module):
    def __init__(self, config, ignore_idx=-1, device=torch.device('cpu')):
        ...
        # using word vector
        # fixme: label_path, it is wrong, should be new vocab
    ...
    def get_batch_data(self, batched_data):
        ...
        def get_batch_data_one(input):
            ...
            list_tokens, list_lens = self.data_loader_bert.get_batched_data(input['bert_text'], self.device)    # , all_tokens
        ...
        for key, value in batched_data.items():
            ...
            # batched_data_torch = get_batch_data_one(value)
            # break
    ...
    def step_train(self, batched_data, inference=False):
        ...
        batched_data_torch = self.get_batch_data(batched_data)  # get input ids
        # ************************************
        # return batched_data_torch
        # ************************************
