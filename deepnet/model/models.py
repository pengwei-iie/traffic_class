...
class BertConCap(nn.Module):
    ...
    def predict(self, batched_data, is_train: bool = True):
        ...
        for k, v in batched_data.items():
            ...
            #     continue
            list_tokens, list_lens = v[0], v[1]     # list_lens -- len of subword
            rep_text, output_tensor_pad = self.bert(list_tokens, list_lens) # (batch, list_lens.item())
            reps[k] = rep_text
            output_pad_words[k] = output_tensor_pad

        # cal capsule fixme: task-adapter
        ...
        for task, label_item in self.tasks_label.items():
            ...
            n_label = label_item['value']
            list_prob, list_r_s = [], []
            for i in range(n_label):    # prob_tmp to Eqn.4, scores; r_s_tmp,list_r_s to Eqn.5, vectors
            ...
            list_r_s = torch.stack(list_r_s)    # 2tasks, 3batches, 768
            list_sim = torch.sum(reps[self.anchor] * list_r_s, 2).t()   # (2tasks, 3batches, 768) (3batches, 768) Eqn.6
            prob = torch.stack(list_prob).squeeze(-1).t()               # Eqn.7
            prob = prob.clamp(min=1e-9, max=1.0)
            dict_probs[task] = prob
            dict_sims[task] = list_sim

        return dict_probs, dict_sims, reps                              # Eqn.7 and Eqn.6
    ...
    def forward(self, batched_data, ignore_idx, is_train: bool = True):
        dict_probs, dict_sims, reps = self.predict(batched_data, is_train)      # return Eqn.7 and 6
