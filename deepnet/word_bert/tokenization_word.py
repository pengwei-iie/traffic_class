...
class WordFullTokenizer(FullTokenizer):
    ...
    def tokenize_with_length(self, text):
        split_tokens = []
        tokens, len_tokens = self.basic_tokenizer.tokenize(text)    # if wanna add new tokens, frequency in here, (tokens)
        len_tokens_shift = len_tokens.copy()

        for idx, token in enumerate(tokens):
            sub_tokens = self.wordpiece_tokenizer.tokenize(token)   # sub_tokens is obtained by the vocab
        ...
        return split_tokens, len_tokens_shift   # add return tokens
...
class WordBasicTokenizer(BasicTokenizer):
    ...
    def tokenize(self, text):
        ...
        for idx, token in enumerate(orig_tokens):
            ...
            if len(tokens_split_punc) > 1:  # calculate length
                def find_idx(lens, num):
