...
class WordBertDataset():
    ...
    def get_idx(self, text):
        ...
        w1 w2 w3 wn
        """
        tokens, lens = self.tokenizer.tokenize_with_length(text)    # obtain split tokens: tokens, lens, before_tokens = self.tokenizer.tokenize_with_length(text)
        tokens = tokens[: self.max_seq_len]
        tokens = ["[CLS]"] + tokens
        tokens = self.tokenizer.convert_tokens_to_ids(tokens)
        # trunk lens fixme: tokens are trunk, so the lenth should be trunk too
        sum_len = 1
        lens_trunk = []
        ...
        assert len(tokens) == np.sum(lens_trunk) + 1    # add 1 cause cls
        return tokens, lens_trunk       # return before_tokens

    def get_batched_data(self, list_text, device):
        list_tokens, list_lens, all_fre_tokens = [], [], []
        for text in list_text:
            tokens, lens = self.get_idx(text)   # , before_tokens
            list_tokens.append(tokens)
            list_lens.append(lens)
            # all_fre_tokens.extend(before_tokens)
        # list_tokens = [self.get_idx(text) for text in list_text]
        dict_re = text_classification_collate_fn(list_tokens, device=device)
        return dict_re, list_lens   # , all_fre_tokens
