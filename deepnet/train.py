...
class TrainModel(object):
    def __init__(self, config, device=torch.device('cpu')):
        ...
        self.config = config
        self.num_loss = config.n_loss   # fixme: return three loss funcs
        ...
        config_label = dict()
        config.tasks_label = json.load(open(config.label_path, 'r'))    # six tasks
    ...
    def run(self, config):
        ...
        tokens = []
        for step in tqdm(range(start_iter, config.max_checkpoint * config.per_checkpoint)):
            if step % config.per_checkpoint == 1:
            # if False:
                ...
                #     exit()
            # **********************
            # token = TrainModel.train(self.model, self.data_train)
            # tokens.extend(token)
            # if step > 300:
            #     # 创建一个空字典来存储单词和出现次数
            #     word_count = {}
            ...
            #     # 遍历list中的每个单词
            #     for word in tokens:
            #         # 如果单词已经在字典中，就把它的出现次数加一
            #         if word in word_count:
            #             word_count[word] += 1
            #         # 如果单词不在字典中，就把它添加到字典中，并把它的出现次数设为一
            #         else:
            #             word_count[word] = 1
            #
            #     # 打印字典
            #     print(word_count)
            #     # 创建一个空列表来存储字典中的前1000个词
            #     word_list = []
            #
            #     # 遍历字典中的每个键值对
            ...
            #
            #     # 如果列表的长度大于1000，就只保留前1000个元素
            #     if len(word_list) > 1000:
            #         word_list = word_list[:1000]
            #
            #     # 打开一个名为words.txt的文件，如果不存在就创建一个
